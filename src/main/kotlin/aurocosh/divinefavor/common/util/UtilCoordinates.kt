...
object UtilCoordinates {
    ...
    fun getSphereOutline(center: BlockPos, radiusInternal: Int, radiusExternal: Int): Sequence<BlockPos> {
        val cubeCoordinates = getBlocksInSphere(center, radiusExternal)
        val radiusSq = radiusInternal * radiusInternal
        return cubeCoordinates.filter { pos -> center.distanceSq(pos) >= radiusSq }
    }

    fun getBlocksInSphere(center: BlockPos, radius: Int): Sequence<BlockPos> {
        val cubeCoordinates = getBlocksInRadius(center, radius, radius, radius)
        val radiusSq = radius * radius
        return cubeCoordinates.S.filter { pos -> center.distanceSq(pos) <= radiusSq }
    }
    ...
    fun getRandomBlockInRange(center: BlockPos, radius: Int, limit: Int, predicate: (BlockPos) -> Boolean): BlockPos? {
        ...
        return findPosition(center, limit, predicate, { it.down() })
                ?: findPosition(center, limit, predicate, { it.up() })
    }
