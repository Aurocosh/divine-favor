...
class EntityRopeGlowingNode(world: World) : EntityRopeNodeBase(world) {
    ...
    override fun registerPickUp(player: EntityPlayer) {
        UtilPlayer.addStackToInventoryOrDrop(player, ItemStack(ModItems.rope_glowing, 1))
    }

    override fun canDropNewNode(player: EntityPlayer): Boolean {
        val (slotIndex, stack) = UtilPlayer.findStackInInventory(player) { element -> !element.isEmpty && element.item === ModItems.rope_glowing }
        if (slotIndex == -1)
            return false
        stack.shrink(1)
