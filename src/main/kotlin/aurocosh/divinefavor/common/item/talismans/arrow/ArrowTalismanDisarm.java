...
public class ArrowTalismanDisarm extends ItemArrowTalisman {
    ...
    private void moveStackToMainInventory(EntityPlayer player, int slot) {
        ...
        if (stack.isEmpty())
            return;
        SlotData slotData = UtilPlayer.INSTANCE.findStackInMainInventory(player, ItemStack::isEmpty);
        if (slotData.isValid())
            UtilPlayer.INSTANCE.swapStacks(player, slot, slotData.getSlotIndex());
    }
}
