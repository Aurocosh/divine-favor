...
class ItemRitualPouch : ModItem("ritual_pouch", "ritual_pouch", ConstMainTabOrder.OTHER_ITEMS) {
    init {
        setMaxStackSize(1)
    ...
    override fun onItemRightClick(worldIn: World?, playerIn: EntityPlayer, hand: EnumHand): ActionResult<ItemStack> {
        ...
        playerIn.openGui(DivineFavor, ConstGuiIDs.RITUAL_POUCH, worldIn!!, playerIn.posX.toInt(), playerIn.posY.toInt(), playerIn.posZ.toInt())
    ...
    override fun initCapabilities(item: ItemStack, nbt: NBTTagCompound?): ICapabilityProvider? {
        return if (item.item === ModItems.ritual_pouch) RitualPouchProvider() else null
    }
