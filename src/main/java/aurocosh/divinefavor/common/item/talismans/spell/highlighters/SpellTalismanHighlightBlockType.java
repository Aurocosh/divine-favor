...
public class SpellTalismanHighlightBlockType extends SpellTalismanHighlightBlockPredicated {
    public SpellTalismanHighlightBlockType(String name, ModSpirit spirit, EnumSet<SpellOptions> options, Color3f color3f, SenseBlock senseBlock, HighlightPredicate predicate) {
        super(name, spirit, options, color3f, senseBlock, predicate);
        if (predicate == HighlightPredicate.BLOCK)
    ...
    protected boolean validate(TalismanContext context) {
        return predicate != HighlightPredicate.BLOCK;
    }
}
