...
class SpiritData {
    ...
    private fun getContracts(contractsStackHandler: ItemStackHandler): List<ItemContract> {
        return contractsStackHandler.asSequence()
                .filter(ItemStack::isNotEmpty)
                .map(this::getContractsFromStack)
                .flatten()
                .map { it.item as ItemContract }
                .toList()
    }
    ...
    fun refreshContracts() {
        ...
        for (i in contractsStackHandlers.indices) {
            ...
            val contracts = getContracts(stackHandler)

            val favorContracts = UtilList.select(contracts, ItemFavorContract::class.java)
            for (contract in favorContracts)
